...
@Component
public class RegCreaProgNotificaBuilder extends AbstractNotificaBuilder implements NotificaBuilderInterface {
	...
	protected List<Notifica> buildNotifiche(EventoDecodedDto evento, EventoTipoDecodedDto dettaglioEvento)throws Exception {
		List<Notifica> notifiche = new ArrayList<>();
			...
			String dataIn = dateTimeformatter.format(enteFase.getFaseInizio());
			String dataFin = dateTimeformatter.format(enteFase.getFaseFine());
			...
			String emailCorpo = email.getTemplateCorpo()
					.replace("<anno>", anno)
					.replace("<DataIn>", dataIn)
					.replace("<DataFin>", dataFin);
